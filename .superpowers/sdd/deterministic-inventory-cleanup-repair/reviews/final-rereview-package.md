# Whole-change no-git final re-review package
...
function handleDraftEvent(payload, generationRequest) {
  ...
  try {
    generationRequest.hasAttemptedDraft = true;
...
form.addEventListener("submit", async (event) => {
  ...
  const generationRequest = { hasAttemptedDraft: false };
  ...
  try {
    ...
    if (!generationRequest.hasAttemptedDraft) {
      renderEpoch += 1;
      clearCurrentModel();
